...
class ApretasteMoney {
	...
	static function getPaymentWarning($email, $datefrom = null, $dateto = null){
		...
		$cards = q("SELECT * FROM dispatchers_cards_without_pay WHERE dispatcher='$email' where true " . (is_null($datefrom) ? "" : " and date >= $datefrom'::date") . " ". (is_null($datefrom) ? "" : " and date <= $dateto'::date") . " order by date;");
		
		$from_date = q("SELECT min(date) as d FROM dispatchers_cards_without_pay WHERE dispatcher='$email' WHERE " . (is_null($datefrom) ? "" : " and date >= $datefrom'::date") . ";");
