...
/*
 * How to use? $server = new phpHotMapServer(); // Server instance $server->addMethod("getEnterprise", "reu"); // Add public method $server->addMethod("Company::getEmployees", true); // Add private method $server->addData("Company", array("name" => "My Company", "phone" => "(444)-485758")); // Add some data $server->go();
 */
class phpHotMapServer {
	...
	public function go(){
		...
		if (isset($_GET['execute'])) {
			...
			/*
			 * a b !a || b ------------------- t t t t f f f f t f t t
			 */
