...
class rpFileSystem {
	 ...
	 * @param $mapping An optional array of extension to media type mappings in the form
	 *        'extension1|extension2|...' => 'type'.
	 *       
	 * @return The internet media type registered for the extension or application/octet-stream for unknown extensions.
	 */
	...
	static function listFiles($from, $ext, $first = true, $e = -1){
		...
		foreach ( $dir as $entry ) {
			...
			if ($e == 4)
				$e = 0;
			if ($first && $is_cli)
				rpCLI::msg("Looking for $ext files...", $esp[$e], true, true);
		...
		if ($first) {
			$cc = count($lstfiles);
			if ($is_cli)
				rpCLI::msg("There were $cc $ext files found in $from");
		}
		return $lstfiles;
	}
}
