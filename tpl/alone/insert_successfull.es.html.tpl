...
<span style="{$font};font-size:small; float: left;">
	...
	<a href="mailto:{$reply_to}?subject=CAMBIAR {$ticket} {$title}&body={$body}">Cambiar el anuncio</a>{$splitter}
        <a href="mailto:{$reply_to}?subject=QUITAR {$ticket}">Quitar el anuncio</a>{$splitter}
	<a href="mailto:{$reply_to}?subject=ESTADO">Consultar mi estado</a>{$splitter}
        <a href="mailto:{$reply_to}?subject=EXCLUYEME">Excluirme de {$apretaste}</a>
</span>
