	...
	<div class="anuncio">
		<table cellspacing = "0" cellpadding="0" width="100%">
			<tr>
				...
				<a href = "mailto:{$reply_to}?subject=ANUNCIO {$id}&body=Presione o haga clic en Enviar para obtener el anuncio seleccionado." style="{$font}; font-size: 12px;"><b>{^title}</b></a>?$price {$splitter} <label style="{$font};{$style-price}; width: 30px; text-align:right;">{$price}&nbsp;{$currency}</label> $price? &nbsp;|&nbsp;
				<span style="font-size: small;">
					?$phones Tel: <b>{$phones.0}</b> {$splitter} $phones?				
